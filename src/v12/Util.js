...
class Util extends null {
  static resolveStyle(style, turnit) {
    if (!style) throw new TypeError('MISSING_BUTTON_STYLE: Please provide a button style.');
  ...
  static checkButton(data) {
    ...
    if (!data.style) throw new TypeError('MISSING_BUTTON_STYLE: Please provide a button style.');

    if (!data.label && !data.emoji) throw new TypeError('NO_BUTTON_LABEL_OR_EMOJI: Please provide a button label and/or an emoji.');
    ...
    if (data.style === MessageButtonStyles['url'] && !data.url)
      throw new TypeError('MISSING_BUTTON_URL: A URL must be provided for a URL Button.');

    if (data.style !== MessageButtonStyles['url'] && data.url) throw new TypeError('BUTTON_STYLE_MISMATCH: A URL can only be assigned to URL Button (.setStyle(\'url\')');

    if (data.style === MessageButtonStyles['url'] && data.custom_id)
      throw new TypeError('UNEXPECTED_CUSTOM_ID: A URL Button can not have custom Id');

    if (data.style !== MessageButtonStyles['url'] && !data.custom_id) throw new TypeError('MISSING_BUTTON_ID: Please provide a button id');
  ...
  static checkMenu(data) {
    ...
    if (!data.custom_id) throw new Error('MISSING_MENU_ID: Please provide a menu id.');
    ...
    if (data.placeholder && data.placeholder.length > 100)
      throw new Error(
        `EXCEEDED_MENU_PLACEHOLDER_CHARACTER_LIMIT: The maximum length of MessageMenu.placeholder is 100 characters, received ${data.placeholder.length} instead.`,
      );
    ...
    if (data.min_values && (data.min_values > 25 || data.min_values < 0))
      throw new Error(`INVALID_MENU_MIN_VALUES: MessageMenu.minValues must be between 0 and 25.`);
  ...
  static checkMenuOptions(data) {
    if (!Array.isArray(data)) throw new Error('INVALID_OPTIONS: Provided select menu options must be an array.');

    if (data.length < 1) throw new Error('MISSING_MENU_OPTIONS: Please provide at least one MessageMenu option.');

    if (data.length > 25) throw new Error(`EXCEEDED_MENU_OPTIONS_LIMIT: The limit of MessageMenu.options is 25, you provided ${options.length} options.`);

    let hasDefault = false;
    data.map((d) => {
      if (!d.label) throw new Error('MISSING_MENU_LABEL: Please provide a menu label.');
      ...
      if (d.label.length > 25)
        throw new Error(
          `EXCEEDED_MENU_LABEL_CHARACTERS_LIMIT: The maximum length of MessageMenuOption.label is 25 characters, received ${d.label.length} instead.`,
        );

      if (!d.value) throw new Error('MISSING_MENU_VALUE: Please provide a menu value.');
      ...
      if (d.value.length > 100)
        throw new Error(
          `EXCEEDED_MENU_VALUE_CHARACTER_LIMIT: The maximum length of MessageMenuOption.value is 100 characters, received ${d.value.length} instead.`,
        );
      ...
      if (d.description && d.description.length > 50)
        throw new Error(
          `EXCEEDED_MENU_DESCRIPTION_CHARACTER_LIMIT: The maximum length of MessageMenuOption.description is 100 characters, received ${d.description.length} instead.`,
        );
